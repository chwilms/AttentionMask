...
template <typename Dtype>
void TopKOldLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
  ...
  // choose first k_
  sort(ids_cpu_data, ids_cpu_data + k2_);
...
template <typename Dtype>
void TopKOldLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
...
INSTANTIATE_LAYER_GPU_FUNCS(TopKOldLayer);
